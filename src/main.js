...
var puntajeBarra1 = 0,
  puntajeBarra2 = 0,
  metaPuntaje = 5; // Variables de puntaje
var nombreJugador1 = "Jugador 1",
  nombreJugador2 = "Jugador 2"; // Nombre de los jugadores
var menuActual = document.querySelector(".menu-principal"),
...
function gameLoop(time) {
  ...
  pelota.setX(
    pelota.getX() +
      (pelota.getDireccionX() == "izquierda" ? -1 : 1) *
        pelota.getVelocidadX() *
        timeDelta
  );
  pelota.setY(
    pelota.getY() +
      (pelota.getDireccionY() == "arriba" ? -1 : 1) *
  ...
  // Borde de abajo
  if (
    pelota.getY() + pelota.getRadio() / 2 >=
      render.canvasAlto - render.grosorBorde / 2 &&
    pelota.getDireccionY() === "abajo"
  ) {
  ...
  // Borde de arriba
  if (
    pelota.getY() - pelota.getRadio() / 2 <= render.grosorBorde / 2 &&
    pelota.getDireccionY() === "arriba"
  ) {
  ...
  if (
    barra1.getX() < pelota.getX() &&
    barra1.getX() + barra1.getAncho() > pelota.getX() - pelota.getRadio() / 2 &&
    pelota.getY() >
      barra1.getY() - barra1.getLargo() / 2 - pelota.getRadio() / 2 &&
    pelota.getY() <
      barra1.getY() + barra1.getLargo() / 2 + pelota.getRadio() / 2 &&
    ...
    pelota.setVelocidadY(
      ((pelota.getY() < barra1.getY()
        ? barra1.getY() - pelota.getY()
        : pelota.getY() - barra1.getY()) /
        (barra1.getLargo() / 2 + pelota.getRadio() / 2)) *
        (maxVelocidadY - minVelocidadY) +
        minVelocidadY
    );
  ...
  if (
    barra2.getX() > pelota.getX() &&
    barra2.getX() - barra2.getAncho() < pelota.getX() + pelota.getRadio() / 2 &&
    pelota.getY() >
      barra2.getY() - barra2.getLargo() / 2 - pelota.getRadio() / 2 &&
    pelota.getY() <
      barra2.getY() + barra2.getLargo() / 2 + pelota.getRadio() / 2 &&
    ...
    pelota.setVelocidadY(
      ((pelota.getY() < barra2.getY()
        ? barra2.getY() - pelota.getY()
        : pelota.getY() - barra2.getY()) /
        (barra2.getLargo() / 2 + pelota.getRadio() / 2)) *
        (maxVelocidadY - minVelocidadY) +
        minVelocidadY
    );
    pelota.setVelocidadX(
  ...
  if (
    pelota.getX() + pelota.getRadio() / 2 >=
    render.canvasAncho - render.grosorBorde / 2
  ) {
    ...
    pelota.setVelocidadX(
      (render.canvasAncho * (numeroAleatorio(4, 7) / 10)) / SEGUNDO
    );
    pelota.setVelocidadY(
      (render.canvasAncho * (numeroAleatorio(2, 7) / 10)) / SEGUNDO
    );
  }
...
function gameOver(ganador) {
  ...
  render.renderizar();
  // Mostrar ganador
  document.querySelector(".puntaje-jugando").style.display = "none";
  document.querySelector(".puntaje-final-1").textContent = puntajeBarra1;
  document.querySelector(".puntaje-final-2").textContent = puntajeBarra2;
...
function empezar() {
  ...
  document.querySelector(".puntaje-jugando").style.display = "flex";
  update = window.requestAnimationFrame(gameLoop);
...
function setValoresIniciales() {
  ...
  lastTick = 0;
  render.setJugando(false);
  minVelocidadX = (render.canvasAncho * 0.4) / SEGUNDO;
  maxVelocidadX = (render.canvasAncho * 0.7) / SEGUNDO;
  minVelocidadY = (render.canvasAncho * 0.2) / SEGUNDO;
  maxVelocidadY = (render.canvasAncho * 0.7) / SEGUNDO;
  cancelAnimationFrame(update);
  // Establecer objetos
  // --- Barra 1 ---
...
document
  .querySelector(".boton-un-jugador")
  .addEventListener("click", unJugador);
document
  .querySelector(".boton-multijugador")
  .addEventListener("click", multijugador);
document
  .querySelector(".boton-como-jugar")
  .addEventListener("click", comoJugar);
document
  .querySelector(".boton-opciones")
  .addEventListener("click", mostrarOpciones);
document
  .querySelector(".boton-local")
  .addEventListener("click", multijugadorLocal);
document
  .querySelector(".boton-online")
  .addEventListener("click", multijugadorOnline);
document
  .querySelector(".boton-pantalla-completa")
  .addEventListener("click", pantallaCompleta);
document
  .querySelector(".boton-reintentar")
...
window.addEventListener("load", () => {
  ...
  if (window.innerWidth < 990) {
    document
      .querySelector(".contenedor-guia-desktop")
      .classList.remove("guia-activa");
    document
      .querySelector(".contenedor-guia-movil")
      .classList.add("guia-activa");
  } else {
    document
      .querySelector(".contenedor-guia-movil")
      .classList.remove("guia-activa");
