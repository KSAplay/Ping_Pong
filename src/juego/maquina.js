...
export function moverBarra2() {
  ...
  if (pelota.getDireccionX() === "derecha" && pelota.getX() > canvasAncho / 2) {
    if (pelota.getDireccionY() === "arriba") {
      if (
        predecirPelotaEnY() >
        barra2.getY() + barra2.getLargo() / 2 + dificultadBarra2
      ) {
        barra2.mover("abajo");
      } else if (
        predecirPelotaEnY() <
...
function predecirPelotaEnY() {
  let vecesX =
    pelota.getDireccionX() === "derecha"
      ? (canvasAncho - pelota.getX()) / pelota.getVelocidadX()
      : pelota.getX() / pelota.getVelocidadX();
  let distanciaY = pelota.getVelocidadY() * vecesX;
  let posY =
    pelota.getDireccionY() === "arriba"
      ? pelota.getY() - distanciaY
      : pelota.getY() + distanciaY;
  return posY;
}
...
function moverBarra1() {
  ...
  if (
    ...
    pelota.getX() < canvasAncho / 2
  ) {
    if (pelota.getDireccionY() === "arriba") {
      if (
        predecirPelotaEnY() >
        barra1.getY() + barra1.getLargo() / 2 + dificultadBarra1
      ) {
        barra1.mover("abajo");
      } else if (
        predecirPelotaEnY() <
