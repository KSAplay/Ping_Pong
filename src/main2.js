...
function gameLoop(time){
    ...
    lastTick = time;
    pelota.setX(pelota.getX() + ((pelota.direccion.x == "izquierda") ? -1 : 1) * pelota.velocidadX * timeDelta);
    pelota.setY(pelota.getY() + ((pelota.direccion.y == "arriba") ? -1 : 1) * pelota.velocidadY * timeDelta);
    ...
    // Borde de abajo
    if(pelota.y + pelota.radio/2 >= render.canvasAlto - render.grosorBorde/2){
        pelota.direccion.y = "arriba";
        sonido.reproducir(sonido.reboteBorde);
    }
    // Borde de arriba
    if(pelota.y - pelota.radio/2 <= render.grosorBorde/2){
        pelota.direccion.y = "abajo";
        sonido.reproducir(sonido.reboteBorde);
    }
    // ---------- Detecta la pelota si rebota en la barra 1 ----------
    if(barra1.getX() < pelota.getX()
        && barra1.getX() + barra1.getAncho() > pelota.getX() - pelota.radio/2
        && pelota.y > barra1.getY() - barra1.getLargo()/2 - pelota.radio/2
        && pelota.y < barra1.getY() + barra1.getLargo()/2 + pelota.radio/2){
        
        pelota.direccion.x = "derecha";
        // Cambia la velocidadX de la pelota dependiendo de donde golpea con la barra 1
        pelota.velocidadY = ((pelota.y < barra1.getY()) ? 
            barra1.getY()-pelota.y : pelota.y-barra1.getY())/(barra1.getLargo()/2+pelota.radio/2)*(maxVelocidadY - minVelocidadY)+minVelocidadY;
        pelota.velocidadX = ((pelota.y < barra1.getY()) ? 
            pelota.y-(barra1.getY()-barra1.getLargo()/2-pelota.radio/2) : (barra1.getY()+barra1.getLargo()/2+pelota.radio/2)-pelota.y)/(barra1.getLargo()/2+pelota.radio/2)*(maxVelocidadX - minVelocidadX)+minVelocidadX;
    ...
    if(barra2.getX() > pelota.getX()
        && barra2.getX() - barra2.getAncho() < pelota.getX() + pelota.radio/2
        && pelota.y > barra2.getY() - barra2.getLargo()/2 - pelota.radio/2
        && pelota.y < barra2.getY() + barra2.getLargo()/2 + pelota.radio/2){
        
        pelota.direccion.x = "izquierda";
        // Cambia la velocidadX de la pelota dependiendo de donde golpea con la barra 2
        pelota.velocidadY = ((pelota.y < barra2.getY()) ? barra2.getY()-pelota.y : pelota.y-barra2.getY())/(barra2.getLargo()/2+pelota.radio/2)*(maxVelocidadY - minVelocidadY)+minVelocidadY;
        pelota.velocidadX = ((pelota.y < barra2.getY()) ? pelota.y-(barra2.getY()-barra2.getLargo()/2-pelota.radio/2) : (barra2.getY()+barra2.getLargo()/2+pelota.radio/2)-pelota.y)/(barra2.getLargo()/2+pelota.radio/2)*(maxVelocidadX - minVelocidadX)+minVelocidadX;
        // Reproduce audio de rebote con la barra
        sonido.reproducir(sonido.reboteBarra);
    }

    // ---------- Detecta si la pelota golpea con el borde detras de las barras ----------
    if(pelota.getX() + pelota.radio/2 >= render.canvasAncho - render.grosorBorde/2){
        pelota.x = barra1.getX() + barra1.getAncho() + pelota.radio/2;
        pelota.y = barra1.getY();
        puntajeBarra1++;
    ...
    if(pelota.getX() - pelota.radio/2 <= render.grosorBorde/2){
        pelota.x = barra2.getX() - barra2.getAncho() - pelota.radio/2;
        pelota.y = barra2.getY();
        puntajeBarra2++;
...
function barraIA(){
    if(pelota.direccion.x === "derecha" && pelota.getX() > render.canvasAncho/2){
        if(barra2.getY() < pelota.y){
...
function setValoresIniciales(){
    ...
    let px = numeroAleatorio(0,1) ? "izquierda" : "derecha";
    let py = numeroAleatorio(0,1) ? "arriba" : "abajo";
    pelota.setDireccion({px, py});
